...
class ZTicketExportXlsx(models.AbstractModel):
    ...
    def _get_ws_params(self, wb, data, sessions):
        session_template = {
            ...
            "responsable": {
                ...
                "data": {"value": self._render("session.user_id.display_name")},
                "width": 20,
            },
            "total_ttc": {
            ...
            "date": {
                "header": {"value": "Date"},
                "data": {"value": self._render("'{:%d-%m-%Y}'.format(line['date'])")},
                "width": 20,
            },
            "compte_lib_code": {
    ...
    def _aggregate_lines(self, lines, line, get_account):
        ...
        if line._name == "account.bank.statement.line":
            nb_transactions = 1
            if line.display_name.endswith("return") or line.display_name.endswith(
                "retour"
            ):
                nb_transactions = 0
    ...
    def _zticket_report(self, workbook, ws, ws_params, data, sessions):
        ...
        for session in sessions:
            ...
            for om_line in obj_account_move_lines:
                ...
                self._aggregate_lines(o_m_lines, om_line, lambda x: x.account_id)
